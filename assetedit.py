...
#         self.page = page
#         self.parent = parent
#         self.asset_id = asset_id  # Assume asset_id is passed to edit a specific asset
...
#         # Disabled fields for asset details
...
#         self.asset_location = ft.TextField(label="Location", hint_text="Location", icon=ft.Icons.LOCATION_ON, disabled=True)

#         # File pickers for images and bills
...
#         self.asset_bill_button = ft.ElevatedButton("Upload Bill", icon=ft.Icons.ATTACH_FILE, on_click=lambda e: self.asset_bill.pick_files(allow_multiple=True))
...
#         # Load asset data on init if asset_id is provided
#         if self.asset_id:
...
#         """Fetch asset details from the local database."""
...
#             else:
#                 self.error_popup.content = ft.Text("Asset not found.")
...
#                         VALUES (?, ?, ?, ?)
#                     """, (self.asset_id, img.name, self.attached_image_bytes, time.strftime("%Y-%m-%d %H:%M:%S")))
...
#                         VALUES (?, ?, ?, ?)
#                     """, (self.asset_id, bill.name, self.attached_bill_bytes, time.strftime("%Y-%m-%d %H:%M:%S")))

#             self.local_db.commit()
#             self.success_popup.content = ft.Text("Images and/or bills added successfully!")
#             self.success_popup.open = True
#             self.page.update()
#         except Exception as e:
#             self.local_db.rollback()
#             self.error_popup.content = ft.Text(f"Error saving images/bills: {e}")
#             self.error_popup.open = True
#             self.page.update()
#         finally:
#             cursor.close()
...
#             # Handle images
#             if self.attached_images and hasattr(self, 'attached_image_bytes'):
#                 cursor.execute("SELECT id, image_name FROM asset_images WHERE asset_id = ?", (self.asset_id,))
...
#             # Handle bills
#             if self.attached_bills and hasattr(self, 'attached_bill_bytes'):
#                 cursor.execute("SELECT id, bill_name FROM asset_bills WHERE asset_id = ?", (self.asset_id,))
...
#             self.local_db.commit()
#             self.success_popup.content = ft.Text("Images and/or bills added/updated successfully!")
#             self.success_popup.open = True
#             self.page.update()
#         except Exception as e:
#             self.local_db.rollback()
...
#         try:
#             cursor.execute("BEGIN TRANSACTION")
#             cursor.execute("UPDATE assets SET location = ? WHERE id = ?", (self.asset_location.value, self.asset_id))
...
#                 self.error_popup.open = True
#             self.page.update()  # Ensure UI updates after loading data
#         except Exception as e:
#             self.error_popup.content = ft.Text(f"Error loading asset: {e}")
#             self.error_popup.open = True
#         finally:
...
#             if self.attached_images and hasattr(self, 'attached_image_bytes'):
#                 # Fetch existing images for this asset
#                 cursor.execute("SELECT id, image_name FROM asset_images WHERE asset_id = ?", (self.asset_id,))
#                 existing_images = {row[1]: row[0] for row in cursor.fetchall()}
#                 for img in self.attached_images:
#                     if img.name in existing_images:
#                         # Update existing image if name matches
#                         cursor.execute("""
#                             UPDATE asset_images SET image_data = ?, last_sync = ? WHERE id = ?
#                         """, (self.attached_image_bytes, time.strftime("%Y-%m-%d %H:%M:%S"), existing_images[img.name]))
#                     else:
#                         # Insert new image only if name doesn't exist
#                         cursor.execute("""
#                             INSERT INTO asset_images (asset_id, image_name, image_data, last_sync)
#                             VALUES (?, ?, ?, ?)
#                         """, (self.asset_id, img.name, self.attached_image_bytes, time.strftime("%Y-%m-%d %H:%M:%S")))
...
#     def close_error_popup(self, event):
#         self.error_popup.open = False
#         self.dialog.open = False  # Force close dialog on error
#         self.page.update()
...
#                 existing_images = {row[1]: row[0] for row in cursor.fetchall()}
#                 for img in self.attached_images:
#                     img_name = os.path.basename(img.name)  # Use filename without path for consistency
#                     if img_name in existing_images:
#                         # Update existing image if name matches
#                         cursor.execute("""
#                             UPDATE asset_images SET image_data = ?, last_sync = ? WHERE id = ?
#                         """, (self.attached_image_bytes, time.strftime("%Y-%m-%d %H:%M:%S"), existing_images[img_name]))
#                         print(f"Updated image {img_name} for asset_id {self.asset_id}")
#                     else:
#                         # Insert new image only if name doesn't exist
#                         cursor.execute("""
#                             INSERT INTO asset_images (asset_id, image_name, image_data, last_sync)
...
#                 existing_bills = {row[1]: row[0] for row in cursor.fetchall()}
#                 for bill in self.attached_bills:
#                     bill_name = os.path.basename(bill.name)
#                     if bill_name in existing_bills:
#                         cursor.execute("""
#                             UPDATE asset_bills SET bill_data = ?, last_sync = ? WHERE id = ?
#                         """, (self.attached_bill_bytes, time.strftime("%Y-%m-%d %H:%M:%S"), existing_bills[bill_name]))
#                     else:
#                         cursor.execute("""
#                             INSERT INTO asset_bills (asset_id, bill_name, bill_data, last_sync)
#                             VALUES (?, ?, ?, ?)
...
#             self.error_popup.content = ft.Text(f"Error updating asset: {e}")
#             self.error_popup.open = True
#             print(f"Save failed: {e}")  # Log error for debugging
#         finally:
#             cursor.close()
#             self.dialog.open = False  # Force close dialog
#             self.page.update()
...
#             if self.attached_images and hasattr(self, 'attached_image_bytes'):
#                 # Find the latest image for this asset_id
#                 cursor.execute("SELECT id, image_name FROM asset_images WHERE asset_id = ? ORDER BY id DESC LIMIT 1", (self.asset_id,))
...
#                 if latest_image:
#                     image_id, existing_name = latest_image
#                     # Update the latest image with the new data
#                     cursor.execute("""
...
#                 else:
#                     # Insert new image if no existing image found
#                     cursor.execute("""
#                         INSERT INTO asset_images (asset_id, image_name, image_data, last_sync)
#                         VALUES (?, ?, ?, ?)
#                     """, (self.asset_id, img_name, self.attached_image_bytes, time.strftime("%Y-%m-%d %H:%M:%S")))
...
#             if self.attached_bills and hasattr(self, 'attached_bill_bytes'):
#                 cursor.execute("SELECT id, bill_name FROM asset_bills WHERE asset_id = ? ORDER BY id DESC LIMIT 1", (self.asset_id,))
...
#                 if latest_bill:
#                     bill_id, existing_name = latest_bill
#                     cursor.execute("""
...
#         self.asset_location = ft.TextField(label="Location", hint_text="Location", icon=ft.Icons.LOCATION_ON)
#         self.asset_image = ft.FilePicker(on_result=self.handle_asset_image)
#         self.asset_image_button = ft.ElevatedButton("Select Image", icon=ft.Icons.IMAGE, on_click=lambda e: self.asset_image.pick_files(allow_multiple=False))  # Restrict to one file
#         self.image_display = ft.Image(width=50, height=50, fit="contain")
#         self.warning_text = ft.Text("", color="red")
#         self.asset_bill = ft.FilePicker(on_result=self.handle_bill_image)
#         self.asset_bill_button = ft.ElevatedButton("Upload Bill", icon=ft.Icons.ATTACH_FILE, on_click=lambda e: self.asset_bill.pick_files(allow_multiple=False))  # Restrict to one file
#         self.bill_display = ft.Image(width=50, height=50, fit="contain")
#         self.bill_warning_text = ft.Text("", color="red")
